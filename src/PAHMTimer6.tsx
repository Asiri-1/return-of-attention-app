...
const PAHMTimer6: React.FC<PAHMTimer6Props> = ({
  ...
  const completePractice = () => {
    ...
    // This ensures we use the proper navigation defined in Stage6Wrapper
    onComplete();
  };
  
  // Record session completion for hour tracking
  const recordSessionCompletion = (sessionDuration: number) => {
    // Get existing sessions for stage 6
    const existingSessions = JSON.parse(localStorage.getItem('stage6Sessions') || '[]');
    ...
    localStorage.setItem('stage6Sessions', JSON.stringify(existingSessions));
    ...
    // Store total hours for stage 6
    localStorage.setItem('stage6Hours', totalHours.toString());
  ...
  const handleFastForwardSession = () => {
    ...
    // Log for development
    console.log(`DEV: Fast-forwarded a ${duration}-minute session for Stage 6`);
  ...
  const handleFastForwardStage = () => {
    ...
    // Save all sessions
    localStorage.setItem('stage6Sessions', JSON.stringify(sessions));
    
    // Set total hours to 15
    localStorage.setItem('stage6Hours', '15');
    ...
    // Log for development
    console.log('DEV: Fast-forwarded entire Stage 6 (15 hours)');
  ...
  return (
    <div className="pahm-timer">
      <div className="timer-header">
        ...
        <h1>PAHM Illuminator Practice</h1>
      </div>

...
const PAHMTimer6: React.FC<PAHMTimer6Props> = ({ 
  ...
  const calculateSessionQuality = (counts: typeof pahmCounts, duration: number, completed: boolean) => {
    ...
    let quality = 8.5; // Highest baseline for Stage 6
    
    if (presentMomentAwareness >= 97) quality += 1.5;
    else if (presentMomentAwareness >= 90) quality += 1;
    else if (presentMomentAwareness < 80) quality -= 1;
    ...
    if (interactionRate > 6) quality -= 0.5; // Very few interruptions expected in Stage 6
    else if (interactionRate < 0.5) quality += 0.5;
  ...
  const handleTimerComplete = useCallback(() => {
    ...
    const sessionData = {
      ...
      notes: `Stage 6 PAHM practice with ${Object.values(pahmCounts).reduce((a, b) => a + b, 0)} attention observations. ${presentPercentage}% present-moment awareness. Focus: Subduing Subtle Distractions and achieving single-pointed concentration.`,
      presentPercentage,
      environment: {
    ...
    // 🔥 ENHANCED: Stage 6 specific post-practice reflection with full analytics
    ...
    const insightMessage = presentPercentage >= 97 
      ? "🎯 Outstanding single-pointed attention! Jhana-like states emerging." 
      : presentPercentage >= 90 
      ? "✨ Excellent progress subduing subtle distractions and developing effortless concentration." 
      : "🌱 Building refined awareness and single-pointed attention skills.";
...
🎯 Stage 6 develops single-pointed attention by subduing even the most subtle distractions. This creates access to jhana states and effortless concentration.`,
      emotion: isFullyCompleted ? 'accomplished' : 'content',
      energyLevel: sessionQuality >= 8 ? 9 : sessionQuality >= 6 ? 7 : 6,
      tags: ['pahm-practice', 'stage-6', 'single-pointed-attention', 'subtle-distractions', posture.toLowerCase(), '3x3-matrix'],
      gratitude: [
  ...
  const handleCompleteEarly = () => {
    if (window.confirm('Complete this session early? Note: Stage 6 practice is most effective with full duration for achieving single-pointed attention.')) {
      handleTimerComplete();
    }
  };
  ...
  const fastForwardMinutes = (minutes: number) => {
    if (process.env.NODE_ENV !== 'production') {
      ...
      const additionalInteractions = Math.floor(Math.random() * (minutes * 0.8)) + Math.floor(minutes * 0.4); // Very few interactions for Stage 6
      setPahmCounts(prev => ({
        ...prev,
        present: prev.present + Math.floor(additionalInteractions * 0.7), // Very high present-moment awareness
        likes: prev.likes + Math.floor(additionalInteractions * 0.1),
        dislikes: prev.dislikes + Math.floor(additionalInteractions * 0.05),
        future: prev.future + Math.floor(additionalInteractions * 0.04),
        worry: prev.worry + Math.floor(additionalInteractions * 0.02),
        anticipation: prev.anticipation + Math.floor(additionalInteractions * 0.03),
        past: prev.past + Math.floor(additionalInteractions * 0.03),
        nostalgia: prev.nostalgia + Math.floor(additionalInteractions * 0.02),
        regret: prev.regret + Math.floor(additionalInteractions * 0.01)
      }));
      
      console.log(`DEV: Fast-forwarded ${minutes} minutes, added ${additionalInteractions} PAHM interactions (Stage 6 - Single-pointed attention)`);
    }
  };

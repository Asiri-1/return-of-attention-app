...
interface PAHMReflection3Props {
  onComplete: () => void;
  onBack: () => void;
}

const PAHMReflection3: React.FC<PAHMReflection3Props> = ({
  onComplete,
  onBack
}) => {
  ...
  console.log('🔍 PAHMReflection3 - Component loaded');
  console.log('🔍 PAHMReflection3 - userData:', userData);

  // Get the most recent Stage 3 session from practice sessions
  const stage3Sessions = userData?.practiceSessions?.filter(session => session.stageLevel === 3) || [];
  const mostRecentSession = stage3Sessions.length > 0 
    ? stage3Sessions.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())[0]
    : null;

  // 🔍 DEBUG: Log session data
  console.log('🔍 PAHMReflection3 - Stage 3 sessions found:', stage3Sessions.length);
  console.log('🔍 PAHMReflection3 - Most recent session:', mostRecentSession);
  ...
  if (mostRecentSession) {
    ...
    console.log('🔍 PAHMReflection3 - Raw PAHM counts from LocalDataContext (underscore format):', pahmCounts);
    ...
    console.log('🔍 PAHMReflection3 - Using LocalDataContext data');
    console.log('🔍 PAHMReflection3 - Converted PAHM data (camelCase):', pahmTrackingData);
    console.log('🔍 PAHMReflection3 - Conversion validation:', {
    ...
    console.log('🔍 PAHMReflection3 - Duration:', practiceDuration, 'Posture:', posture);

  } else {
    // Fallback to sessionStorage (for backward compatibility)
    console.warn('🚨 PAHMReflection3 - No Stage 3 session found in LocalDataContext, falling back to sessionStorage');
    
    const rawPahmData = JSON.parse(sessionStorage.getItem('pahmTracking') || '{}');
    
    // 🔍 DEBUG: Log sessionStorage fallback data
    console.log('🔍 PAHMReflection3 - SessionStorage fallback data:', rawPahmData);
    ...
    console.log('🔍 PAHMReflection3 - Using sessionStorage fallback');
    console.log('🔍 PAHMReflection3 - Fallback PAHM data:', pahmTrackingData);
    console.log('🔍 PAHMReflection3 - Fallback duration:', practiceDuration, 'Posture:', posture);
  }

  // Calculate total for validation
  const totalPahmCount = Object.values(pahmTrackingData).reduce((sum, count) => sum + count, 0);
  console.log('🔍 PAHMReflection3 - Total PAHM interactions:', totalPahmCount);

  // Handle completion
  const handleComplete = () => {
    console.log('🔍 PAHMReflection3 - Reflection completed');
  ...
  if (totalPahmCount === 0 && !mostRecentSession) {
    console.warn('🚨 PAHMReflection3 - No PAHM data found in either LocalDataContext or sessionStorage');
  }
...
export default PAHMReflection3;
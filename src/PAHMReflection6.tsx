...
const PAHMReflection6: React.FC<PAHMReflection6Props> = ({
  ...
  console.log('🔍 PAHMReflection6 - Component loaded');
  console.log('🔍 PAHMReflection6 - userData:', userData);

  // Get the most recent Stage 6 session from practice sessions
  const stage6Sessions = userData?.practiceSessions?.filter(session => session.stageLevel === 6) || [];
  const mostRecentSession = stage6Sessions.length > 0 
    ? stage6Sessions.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())[0]
    : null;

  // 🔍 DEBUG: Log session data
  console.log('🔍 PAHMReflection6 - Stage 6 sessions found:', stage6Sessions.length);
  console.log('🔍 PAHMReflection6 - Most recent session:', mostRecentSession);
  ...
  if (mostRecentSession) {
    ...
    console.log('🔍 PAHMReflection6 - Raw PAHM counts from LocalDataContext (underscore format):', pahmCounts);
    ...
    console.log('🔍 PAHMReflection6 - Using LocalDataContext data');
    console.log('🔍 PAHMReflection6 - Converted PAHM data (camelCase):', pahmTrackingData);
    console.log('🔍 PAHMReflection6 - Conversion validation:', {
    ...
    console.log('🔍 PAHMReflection6 - Duration:', practiceDuration, 'Posture:', posture);

  } else {
    // Fallback to sessionStorage (for backward compatibility)
    console.warn('🚨 PAHMReflection6 - No Stage 6 session found in LocalDataContext, falling back to sessionStorage');
    
    const rawPahmData = JSON.parse(sessionStorage.getItem('pahmTracking') || '{}');
    
    // 🔍 DEBUG: Log sessionStorage fallback data
    console.log('🔍 PAHMReflection6 - SessionStorage fallback data:', rawPahmData);
    ...
    console.log('🔍 PAHMReflection6 - Using sessionStorage fallback');
    console.log('🔍 PAHMReflection6 - Fallback PAHM data:', pahmTrackingData);
    console.log('🔍 PAHMReflection6 - Fallback duration:', practiceDuration, 'Posture:', posture);
  }

  // Calculate total for validation
  const totalPahmCount = Object.values(pahmTrackingData).reduce((sum, count) => sum + count, 0);
  console.log('🔍 PAHMReflection6 - Total PAHM interactions:', totalPahmCount);

  // Handle completion
  const handleComplete = () => {
    console.log('🔍 PAHMReflection6 - Reflection completed');
  ...
  if (totalPahmCount === 0 && !mostRecentSession) {
    console.warn('🚨 PAHMReflection6 - No PAHM data found in either LocalDataContext or sessionStorage');
  }

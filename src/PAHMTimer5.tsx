...
const PAHMTimer5: React.FC<PAHMTimer5Props> = ({
  ...
  const completePractice = () => {
    ...
    // This ensures we use the proper navigation defined in Stage5Wrapper
    onComplete();
  };
  
  // Record session completion for hour tracking
  const recordSessionCompletion = (sessionDuration: number) => {
    // Get existing sessions for stage 5
    const existingSessions = JSON.parse(localStorage.getItem('stage5Sessions') || '[]');
    ...
    localStorage.setItem('stage5Sessions', JSON.stringify(existingSessions));
    ...
    // Store total hours for stage 5
    localStorage.setItem('stage5Hours', totalHours.toString());
  ...
  const handleFastForwardSession = () => {
    ...
    // Log for development
    console.log(`DEV: Fast-forwarded a ${duration}-minute session for Stage 5`);
  ...
  const handleFastForwardStage = () => {
    ...
    // Save all sessions
    localStorage.setItem('stage5Sessions', JSON.stringify(sessions));
    
    // Set total hours to 15
    localStorage.setItem('stage5Hours', '15');
    ...
    // Log for development
    console.log('DEV: Fast-forwarded entire Stage 5 (15 hours)');

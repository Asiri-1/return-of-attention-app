...
const PAHMTimer3: React.FC<PAHMTimer3Props> = ({
  ...
  const completePractice = () => {
    ...
    // This ensures we use the proper navigation defined in Stage3Wrapper
    onComplete();
  };
  
  // Record session completion for hour tracking
  const recordSessionCompletion = (sessionDuration: number) => {
    // Get existing sessions for stage 3
    const existingSessions = JSON.parse(localStorage.getItem('stage3Sessions') || '[]');
    ...
    localStorage.setItem('stage3Sessions', JSON.stringify(existingSessions));
    ...
    // Store total hours for stage 3
    localStorage.setItem('stage3Hours', totalHours.toString());
  ...
  const handleFastForwardSession = () => {
    ...
    // Log for development
    console.log(`DEV: Fast-forwarded a ${duration}-minute session for Stage 3`);
  ...
  const handleFastForwardStage = () => {
    ...
    // Save all sessions
    localStorage.setItem('stage3Sessions', JSON.stringify(sessions));
    
    // Set total hours to 15
    localStorage.setItem('stage3Hours', '15');
    ...
    // Log for development
    console.log('DEV: Fast-forwarded entire Stage 3 (15 hours)');
